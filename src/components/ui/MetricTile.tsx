...
type Tone = "default" | "green" | "amber" | "red" | "black" | "accent" | "sky";
...
const TONE_CLASS: Record<Tone, string> = {
  ...
  // Non-status KPIs (phase, population, forecast) — brand-coloured so the strip
  // reads as a vibrant operating picture, not a row of blank white cards.
  accent: "border-accent/50 bg-[color-mix(in_oklab,var(--accent)_10%,var(--surface-1))]",
  sky: "border-status-sky/50 bg-[color-mix(in_oklab,var(--status-sky)_10%,var(--surface-1))]",
};
...
// Icon colour tracks the tone so the glyph carries the same vibrant cue as the
// value, instead of staying muted-grey on a coloured card.
  ...
  return (
    <Card padding="md" className={`flex flex-col gap-2 min-h-[6.5rem] ${TONE_CLASS[tone]}`}>
      <div className="flex items-center gap-2 text-[11px] md:text-[10px] uppercase tracking-[0.08em] text-text-muted font-medium">
        <span className={ICON_TONE_CLASS[tone]}>{icon}</span>
        <span>{label}</span>
      </div>
      <div
